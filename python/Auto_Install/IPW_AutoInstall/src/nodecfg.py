...
class NodeCfg :
    ...
    def setCommon(self, json_node) :
        ...
        if common.getOptionValue(json_node, "traffic_ipv6") :
            self._traffic_ipv6, self._traffic_ipv6_prefix = self.parseIp(str(json_node["traffic_ipv6"]), common.C_IPv6_PREFIX_LEN)
        if common.getOptionValue(json_node, "internal_ip") :
            self._internal_ip, self._internal_prefix = self.parseIp(str(json_node["internal_ip"]), common.C_IPv4_PREFIX_LEN)
        if common.getOptionValue(json_node, "heartbeat_ip1") :
            self._heartbeat_ip1, self._heartbeat_prefix = self.parseIp(str(json_node["heartbeat_ip1"]), common.C_IPv4_PREFIX_LEN) 
        if common.getOptionValue(json_node, "heartbeat_ip2") :
            self._heartbeat_ip2, self._heartbeat_prefix = self.parseIp(str(json_node["heartbeat_ip2"]), common.C_IPv4_PREFIX_LEN) 
        if common.getOptionValue(json_node, "provision_ip") :
            self._provision_ip, self._provison_prefix = self.parseIp(str(json_node["provision_ip"]), common.C_IPv4_PREFIX_LEN)
    ...
    def input_username_password(self):
        print "Please Input Information of %s" %(self._hostname)
    ...
    def parseIp(self, ip, prefix) :
        ...
        if ip :
            ...
            if re.search(r'/', ip) :
                ...
                if re.search(r'\.', t_ip):
                    if (1 > prefix or 31 < prefix) :
                        raise Exception("ipv4 Netmask is error")
                if re.search(r':', t_ip):
                    if (1 > prefix or 127 < prefix) :
                        raise Exception("ipv6 Netmask is error")
            common.checkIPFormat(t_ip)
        return t_ip, prefix
...
class SsCfg(NodeCfg) :
    ...
    def validateCfg(self) :
        ...
        if (not cmp(common.C_IPW_MODE_MEDIUM1, common.g_ipw_mode) \
          ...
          and (not self._heartbeat_ip1 or not self._heartbeat_ip2):
            log._file.error("Heartbeat IP can't be empty in Medium1/Medium2 with SS node")
            is_ok = False
...
class PsCfg(NodeCfg) :
    ...
    def validateCfg(self) :
        ...
        if not cmp(common.C_IPW_MODE_MEDIUM2, common.g_ipw_mode) \
          and (not self._heartbeat_ip1 or not self._heartbeat_ip2) :
            log._file.error("Heartbeat IP can't be empty in Medium2 with PS node")
            is_ok = False
        return is_ok
