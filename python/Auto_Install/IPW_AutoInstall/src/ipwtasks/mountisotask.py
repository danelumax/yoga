...
class MountIsoTask(object):
    ...
    def precheck(self):
        ...
        cfg = cfgInstance();
        isoPath = cfg.getInstallPath() + cfg.getIpwIsoName();
    ...
    def _remote_exec(self, cfg) :
        ...
        if not cmp(cfg.getHostName(), self._hostname) :
            command = "python mountiso.py --mount_point=%s --iso_path=%s" %(cfgInstance().getMountPoint(), cfgInstance().getIpwIsoPath())
        else :
            command = "python mountiso.py --mount_point=%s --iso_path=%s" %(cfgInstance().getMountPoint(), cfgInstance().getInstallPath() + cfgInstance().getIpwIsoName())
