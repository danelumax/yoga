...
class FileTransferTask(object):
    ...
    def execute(self):
        ...
        cfg = cfgInstance()
        self._dest_dir = cfg.getUpgradePath()
        self._mount_point = cfg.getMountPoint()
        self._os_iso = cfg.getOsIsoPath()
        self._ipw_iso = cfg.getIpwIsoPath()
        self._logging_conf = "src/logs/logging.conf"
        self._log = "src/logs/log.py"
    ...
    def _copy_file(self, node, mode='ps') : 
        ...
        log._file.debug("Copy log releated files to "+ node.getHostName())
        scp.copy_to(self._logging_conf, self._dest_dir)
        scp.copy_to(self._log, self._dest_dir)
        log._file.debug("Copy Script file to " + node.getHostName())
        file_list = self._getfiles()
        for x in file_list : 
            scp.copy_to(x, self._dest_dir)

        log._file.debug("Copy upgrade rmps to " + node.getHostName())
        cfg = cfgInstance()
        for x in cfg.getUpdateRPMS() :
            scp.copy_to(x, self._dest_dir)
        log._file.debug("<<<")
