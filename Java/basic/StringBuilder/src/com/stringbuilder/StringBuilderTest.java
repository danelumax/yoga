...
/*
如果需要频繁修改字符串 的内容，建议使用字符串缓冲 类（StringBuilder）。


StringBuilder 其实就是一个存储字符 的容器。

笔试题目：使用Stringbuffer无 参的构造函数创建 一个对象时，默认的初始容量是多少？ 如果长度不够使用了，自动增长多少倍？
	StringBuilder 底层是依赖了一个字符数组才能存储字符数据 的，该字符串数组默认 的初始容量是16， 如果字符数组的长度不够使用 死，自动增长1倍。


StringBuilder 是一个存储字符的容器 
...
StringBuilder 与 StringBuffer的相同处与不同处：
	
	相同点：
		1. 两个类都是字符串缓冲类。
		2. 两个类的方法都是一致的。
	不同点：
		1. StringBuilder是线程非安全的,操作效率高, StringBuffer是线程安全的,操作效率低。
		2. StringBuilder 是jdk1.5的时候出现的， StringBuffer是jdk1.0出现的。
...
public class StringBuilderTest {
	...
	public static void main(String[] args) {
		//先使用StringBuilder无参的构造函数创建一个字符串缓冲类。
		...
		*/
		...
		sb.append("javajava");
		System.out.println("查看字符数组的长度："+ sb.capacity());   //32 = 16*2 + 2
		*/
